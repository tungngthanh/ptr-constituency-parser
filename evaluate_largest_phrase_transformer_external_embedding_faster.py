...
def evaluate_anary_tree(model,eval_data,model_tag,vocab_dict,device,with_special_splitting=True, save_dir='save_result/'):
	...
	pred_tree = []
	counting_time = 0
	pointing_score_data_all = []
	splitting_score_data_all = []
	pred_label_all = []
	pred_wordlabel_all = []
	start_time_all = time.time()
	eval_batch_size=10
	for i in range(0, len(eval_data['pointing']), eval_batch_size):
		sentence = eval_data['sents'][i:min(i + eval_batch_size, len(eval_data['pointing']))]
		tag = eval_data['tags'][i:min(i + eval_batch_size, len(eval_data['pointing']))]
		...
		for i in range(len(sentence)):
			pointing_score_data_all.append(pointing_scores.data.cpu().numpy()[i, :len(sentence[i]), :len(sentence[i])])
			splitting_score_data_all.append(
				splitting_scores.data.cpu().numpy()[i, :len(sentence[i]), :len(sentence[i])])
			pred_label_all.append(torch.max(label_scores, 2).indices.tolist()[i][:len(sentence[i])])
			pred_wordlabel_all.append(torch.max(wordlabel_scores, 2).indices.tolist()[i][:len(sentence[i])])
		# pointing_score_data = pointing_scores.data.cpu().numpy()[0:len(sentence)]
		# splitting_score_data= splitting_scores.data.cpu().numpy()[0:len(sentence)]
		counting_time += (time.time() - start_time)
	# print(len(pointing_score_data_all))
	# print("--- %s seconds ---" % (counting_time))
	for i in range(len(eval_data['pointing'])):
		# sentence = [eval_data['sents'][i]]
		...
		pointing_score_data = pointing_score_data_all[i]
		splitting_score_data = splitting_score_data_all[i]
		...
		pred_label = pred_label_all[i]
		pred_wordlabel = pred_wordlabel_all[i]
		# counting_time+=(time.time()-start_time)
